...
def train_and_evaluate_td3(learning_rate=1e-3, gamma=0.99, batch_size=100, train_steps=20000, val_steps=100):
...
if __name__ == "__main__": # batch testing of hyperparameters
    ...
    for i in range(n_tests):    # loop through test schedule and run each test set and record results
        ...
        steps = 20000
        ar, mr = train_and_evaluate_td3(test_set[i][0], test_set[i][1], test_set[i][2],train_steps=steps,val_steps=100)
